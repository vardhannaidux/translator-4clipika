"""
Test applying all proposed fixes in-memory to verify full sentence translation.
"""
import sys, os
...
_orig_phrases = dict(tm.PHRASE_MAPPINGS)
_orig_conjuncts = dict(tm.CONJUNCT_RULES)
_orig_consonants = {k: dict(v) for k, v in tm.CONSONANTS.items()}
_orig_vowels = dict(tm.VOWELS)

# -------------------------------------------------------------
# APPLY FIXES TO IN-MEMORY MAPPINGS FOR TESTING
# -------------------------------------------------------------

# Fix 1: Update 'స్వ' phrase family in PHRASE_MAPPINGS
# Replacing prefix [254, 167, 121] (þ§y) with [250, 136, 121] (ú\x88y)
tm.PHRASE_MAPPINGS['స్వ'] = bytes([250, 136, 121]).decode('cp1252')
tm.PHRASE_MAPPINGS['స్వా'] = bytes([250, 136, 121, 165]).decode('cp1252') # or [250, 136, 121, 176]? wait, let's check
...
# Remove standalone 'లక్ష' from PHRASE_MAPPINGS so 'లక్ష్యం' uses assembly/CONJUNCT_RULES
if 'లక్ష' in tm.PHRASE_MAPPINGS:
    del tm.PHRASE_MAPPINGS['లక్ష']

# Fix 3: PHRASE_MAPPINGS['ఛార్జి'] and ఇంఛార్జి
# Expected for ఇంఛార్జి: [207, 217, 224, 197, 166, 74, 98]
# 'ఛార్జి' in phrase mappings was [77, 113] (wrong).
if 'ఛార్జి' in tm.PHRASE_MAPPINGS:
    del tm.PHRASE_MAPPINGS['ఛార్జి']
tm.CONJUNCT_RULES[(tm.U_JA, tm.M_I, (tm.U_RA,))] = [74, 98]
# Or: (tm.U_JA, tm.M_I, tm.U_RA): [74, 98]
tm.CONJUNCT_RULES[(tm.U_JA, tm.M_I, tm.U_RA)] = [74, 98]
...
tm.CONJUNCT_RULES[('శు', None, None)] = [248, 338, 8240] # wait, 'శు' is sha+u matra -> [248, 140, 137] in assembly, which decodes to [248, 338, 8240]
...
tm.PHRASE_MAPPINGS['రెడ్డి'] = bytes([183, 244, 232, 147, 133, 34]).decode('cp1252') # wait, expected [183, 244, 232, 8230, 34] where 8230 is 133 in cp1252
# Let's check: bytes([183, 244, 232, 133, 34]).decode('cp1252') produces [183, 244, 232, 8230, 34]!
...
tm.CONJUNCT_RULES[(tm.U_JA, tm.M_I, (tm.U_BA,))] = [71, 98]
# Also remove PHRASE_MAPPINGS['జి'] if present
...
# Fix 15: ఉద్దేశం
# [105, 209, 235, 182, 204, 196, 248, 338, 217]
# Let's add PHRASE_MAPPINGS['ఉద్దేశం'] or CONJUNCT_RULES
...
print("=" * 70)
print("TESTING IN-MEMORY FIXES FOR ALL 21 WORDS")
print("=" * 70)
